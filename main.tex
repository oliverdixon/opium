...
% source (designed for XeLateX)

\documentclass{article}

\usepackage[a4paper]{geometry}
\usepackage[bottom]{footmisc} % Push footnotes to the bottom of the page
\usepackage[super]{nth} % Place cardinal markers in the superscript
\usepackage{graphicx,mdframed,xcolor} % Frame environments and Chinese decor
\usepackage{indentfirst} % Indent new paragraphs
\usepackage{xeCJK} % XeLaTeX: Enable CJK support
\usepackage{setspace} % Custom line spacing
\usepackage{fancyhdr,titling,lastpage} % Header and page metadata
\usepackage{polyglossia} % Cyrillic text

\setmainfont[UprightFont = *, %
        ItalicFont = * Slanted, %
        SmallCapsFont = * Caps]{Latin Modern Roman}
\setCJKmainfont[UprightFont = * SC, %
        ItalicFont = * JP]{Noto Serif CJK}
\setCJKmonofont{Noto Serif CJK TC}
\newfontfamily\russianfont{Liberation Serif}[Script=Cyrillic]
...
% Break long URLs which do not contain breaking characters
...
\fancypagestyle{stdhdr}{%
	...
	\fancyhead[R]{\theauthor}
        ...
        \fancyfoot[L]{\textbf{Release Copy --- \today}}
...
\begin{center}
        \textsc{This ``Non-Examined Assessment'' Document is Presented as
        Partial Fulfilment of the \textit{A.Q.A. A-Level History}
        Qualification.}
\end{center}
...
\textit{Eiichi Motono}, native name 本野英一, is a Japanese professor in
...
Chinese name 天國 as \textit{[the] Kingdom of Heaven}, its leader,
